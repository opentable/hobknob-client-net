...
namespace HobknobClientNet
{
    internal class FeatureToggleProvider : IDisposable
    {
        ...
        public FeatureToggleProvider(EtcdClient etcdClient, string applicationName)
        {
            ...
            _applicationUri = $"v1/toggles/{applicationName}";
            _applicationDirectoryKey = new Uri(_applicationUri, UriKind.Relative);
        }
        ...
        {
            return name.Replace($"/{_applicationUri}/", string.Empty);
        }
